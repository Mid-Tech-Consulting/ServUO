    ...
    public class ArenaFight
    {
            ...
            Timer.DelayCall(TimeSpan.FromMilliseconds(250), () =>
                {
                    foreach (var pm in GetFighters())
                    {
                        ...
                        pm.ProcessDelta();
                        /*foreach (var fighter in GetFighters().Where(x => x != pm))
                        {
                            Notoriety.Compute(pm, fighter);
                        }*/
                    }
                });
        }
        ...
        public virtual void EndFight(ArenaTeam winner, ArenaTeam loser, bool forfeit)
        {
            ...
            if (!forfeit) //Forfeits will not give wons/losses!
            {
                ...
                foreach (var team in m_Teams.Select(x => x.Team).Where(t => t != winner))
                {
                    team.Losses++;
                    team.DamageGiven += GetTotalDamageGiven(team);
                    team.DamageTaken += GetTotalDamageTaken(team);

                    //Console.WriteLine("[{0}] DamageGiven: {1}; Damage Taken {2}", team.Name, GetTotalDamageGiven(team), GetTotalDamageTaken(team));
                }

                winner.DamageGiven += GetTotalDamageGiven(winner);
                winner.DamageTaken += GetTotalDamageTaken(winner);
                //Console.WriteLine("[{0}] DamageGiven: {1}; Damage Taken {2}", winner.Name, GetTotalDamageGiven(winner), GetTotalDamageTaken(winner));
        ...
        public void RecordDamageEntries()
        {
            foreach (var team in m_Teams.Select(x => x.Team))
            {
                ...
                {
                    foreach(var de in fighter.DamageEntries.Where(e => !team.Fighters.Contains(e.Damager)))
                    {
            ...
            foreach (var tm in m_Teams.Select(x => x.Team).Where(t => t != team))
            {
                foreach (var fighter in tm.Fighters)
                {
                    if (m_DamageEntries.ContainsKey(fighter))
                    {
                        ...
                        {
                            if (team.Fighters.Contains(kvp.Key))
                            {
                                damage += kvp.Value;
                            }
                        }
        ...
        public virtual void DoTieBreaker()
        {
            ...
            //First, system goes by alive players, then by damage done, then a random pick(unlikely).
            Dictionary<ArenaTeam, int> fighterTable = new Dictionary<ArenaTeam, int>();
            ...
            foreach (var team in m_Teams.Select(x => x.Team))
            {
                ...
                fighterTable[team] = team.Fighters.Where(f => f.Alive).Count();
            }
            ...
            foreach (KeyValuePair<ArenaTeam, int> kvp in fighterTable)
            {
                ArenaTeam t = kvp.Key;
                int stillAlive = kvp.Value;

                if (stillAlive > highest)
                {
                    winner = t;
                    highest = stillAlive;
                }
            ...
            if (winner != null)
            {
                fighterTable.Remove(winner);

                foreach (KeyValuePair<ArenaTeam, int> kvp in fighterTable)
                {
                    if (fighterTable.Count == 1)
                    {
                        runnerUp = kvp.Key;
                        highest = kvp.Value;
                        break;
            ...
            runnerUpTotal = highest;

            if (winner != null && winnerTotal > runnerUpTotal)
            {
                EndFight(winner, runnerUp);
            }
            else
            {
                fighterTable.Clear();
                ...
                foreach (TeamInfo info in m_Teams)
                {
                    if (info.Team == null)
                        continue;

                    fighterTable[info.Team] = GetTotalDamageGiven(info.Team);
                }
                ...
                {
                    List<TeamInfo> list = new List<TeamInfo>(m_Teams);
                    TeamInfo winInfo = list[Utility.Random(list.Count)];
                    winner = winInfo.Team;

                    list.Remove(winInfo);

                    if (list.Count > 0)
                        runnerUp = list[Utility.Random(list.Count)].Team;
                    else
                        runnerUp = null;
        ...
        public virtual bool AddFighter(ArenaTeam team)
        {
            if (team == null)
                return false;
        ...
        public virtual void MoveToStartSpot(Mobile mob, Point3D loc, int idx)
        {
            ...
            mob.MoveToWorld(new Point3D(loc.X, loc.Y + yOffset, loc.Z), m_System.ArenaMap);
            return;
        }
        ...
        public void EjectPlayers(List<PlayerMobile> list, bool clear)
        {
            ...
            foreach (var mob in list)
            {
                ...
                else if (mob.Region.IsPartOf(m_System.FightRegion))
                {
                    ...
                    BaseCreature.TeleportPets(mob, pnt, map);
        ...
        public void CancelFight(CancelReason reason)
        {
            string message;
            switch (reason)
            {
                ...
                case CancelReason.PlayerLogged: message = "The fight has been canceled because one of the participants has logged out."; break;
        ...
        public void AwardWager(ArenaTeam winners)
        {
            ...
            {
                int amount = m_Wager * m_Teams.Count;

                Banker.Deposit(player, m_Wager * m_Teams.Count, false);
                player.SendMessage("Your team takes the purse! {0} gold has been deposited into your account.", amount.ToString("N0"));
            }

            m_Wager = 0;
        }

        public void RemoveWager()
        {
            foreach (var player in GetFighters())
            {
                Banker.Withdraw(player, m_Wager);
                player.SendMessage("A {0} wager has been withdrawn from your bank account.", m_Wager.ToString("N0"));
            }
        }
        ...
        public bool IsParticipant(Mobile mob)
        {
            ...
            return GetFighters().Any(pm => pm == (PlayerMobile)mob);
        }
        ...
        {
            if (m_System.AudienceRegion == null)
                yield break;

            foreach (var pm in m_System.AudienceRegion.GetEnumeratedMobiles().OfType<PlayerMobile>())
            {
                yield return pm;
            }
        }
		...
		public TeamInfo GetTeamInfo(Mobile m)
		{
			...
			foreach(TeamInfo info in m_Teams)
			{
				if(info.Team != null)
				{
					foreach(Mobile mob in info.Team.Fighters)
					{
						if(mob == m)
							return info;
					}
        ...
        public ArenaTeam GetTeam(Mobile m)
        {
            ...
            {
                if (team != null && team.Team != null && team.Team.Fighters.Contains(m))
                {
                    return team.Team;
                }
            }
