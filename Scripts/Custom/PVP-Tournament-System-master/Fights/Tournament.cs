    ...
    public class Tournament : IComparable, IDisposable
    {
                ...
                case TourneyType.Bracketed:
                    {
                        ...
                        {
                            m_LosersBracket.Add(losers);
                            DoEndOfFightMessage(winners, losers);
                        }
        ...
        public void CancelTournamentFight(ArenaTeam a, ArenaTeam b)
        {
            ...
            switch (m_TourneyType)
            {
                ...
                case TourneyType.Bracketed:
                    {
                        if (m_LosersBracket.Contains(a) || m_Round > 1)
                        {
                            EliminateTeam(a);
                            if (m_Round == 1 && m_OrigPartList.Contains(a))
                                m_OrigPartList.Remove(a);
                        ...
                        {
                            m_LosersBracket.Add(a);
                        }

                        if (m_LosersBracket.Contains(b) || m_Round > 1)
                        {
                            EliminateTeam(b);
                            if (m_Round == 1 && m_OrigPartList.Contains(b))
                                m_OrigPartList.Remove(b);
        ...
        public void Register(Mobile creator)
        {
            ...
            if (creator != null && creator.AccessLevel == AccessLevel.Player)
                m_Creator = creator;
        ...
        public void TryJoinTournament(Mobile from)
        {
            ...
            else
            {
                ...
                if (team != null)
                {
                    ...
                    {
                        if (!ArenaHelper.CanRecievePoints(participant, team) && Config.RestrictSameIP && from.AccessLevel == AccessLevel.Player)
                        {
        ...
        public Tournament(GenericReader reader, PVPTournamentSystem sys)
        {
            ...
            if (t.Count > 0)
                Timer.DelayCall(TimeSpan.FromSeconds(10), new TimerStateCallback(BuildParticipantList), new object[] { t, 0 });
            ...
            if (t.Count > 0)
                Timer.DelayCall(TimeSpan.FromSeconds(10), new TimerStateCallback(BuildParticipantList), new object[] { t, 1 });
