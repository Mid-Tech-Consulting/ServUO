...
namespace Server.Items
{
    public class ArenaBell : Item
    {
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            list.Add("Double Click to Send Duel Challenge.");
        }
        ...
        public override void OnDoubleClick(Mobile m)
        {
            ...
            if (m.InRange(GetWorldLocation(), 2))
            {
                if (HasDelay(m))
                {
                    m.SendMessage("You have to wait a little while before broadcasting another message.");
                }
                else if(System != null)
                {
                    BaseGump.SendGump(new BroadcastGump((PlayerMobile)m,
                        (mob, amount) =>
                        {
                            if (amount > 0)
                            {
                                World.Broadcast(0x21, true, String.Format("{0} is offering a {1} wager for a duel at the {2}!", mob.Name, amount.ToString(), System.Name));
                            }
                            else
                            {
                                World.Broadcast(0x21, true, String.Format("{0} is seeking an opponent at the {1}!", mob.Name, System.Name));
                            }
    ...
    public class BroadcastGump : BaseGump
    {
        ...
        public override void AddGumpLayout()
        {
            ...
            AddHtml(10, 15, 280, 40, Center("Add a wager amount for your challenge broadcast, or leave blank for no wager."), false, false);

            AddButton(15, 68, 4005, 4006, 1, GumpButtonType.Reply, 0);
            AddTooltip("Broadcast Challenge Message");
        ...
        public override void OnResponse(RelayInfo info)
        {
            ...
            User.SendMessage("Broadcast canceled.");
        }
    }
}