...
namespace Server.TournamentSystem
{
    public class FightRegion : BaseRegion
    {
        ...
        {
            if (System == null || m.AccessLevel > AccessLevel.Player)
            ...
            if (m is PlayerMobile)
            {
                if (System.CurrentFight.ArenaFightType != ArenaFightType.CaptureTheFlag)
                {
                    ...
                    if (f.ArenaFightType == ArenaFightType.LastManStanding)
                    {
                        ...
                        {
                            killer.SendMessage(ArenaHelper.ParticipantMessageHue, "You have eliminated {0} from the arena fight. You have {1} {2} left.", m.Name, left, left == 1 ? "team" : "teams");
        ...
        public override bool AllowBeneficial(Mobile from, Mobile target)
        {
            ...
            if (System.CurrentFight != null)
            {
                ...
                foreach (TeamInfo info in System.CurrentFight.Teams)
                {
                    ...
                    if (info.Team.Fighters.Contains(from) && info.Team.Fighters.Contains(target))
                        return true;
                }
            }
        ...
        public override bool AllowHarmful(Mobile from, IDamageable d)
        {
            ...
            if (System != null && target != null && System.CurrentFight != null)
            {
                ...
                foreach (TeamInfo info in System.CurrentFight.Teams)
                {
                    ...
                    if (team.Fighters.Contains(from) && team.Fighters.Contains(target))
                        return false;

                    if ((team.Fighters.Contains(from) && !team.Fighters.Contains(target)) || (!team.Fighters.Contains(from) && team.Fighters.Contains(target)))
                        return true;
                }
            }
