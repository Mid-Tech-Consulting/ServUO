...
namespace Server.TournamentSystem
{
    public class ChooseFightTypeGump : BaseTournamentGump
    {
        ...
        public override void AddGumpLayout()
        {
            ...
            AddButton(15, 50, 4005, 4007, 1, GumpButtonType.Reply, 0);
            AddTooltip(Localizations.GetLocalization(34));
            AddHtml(48, 50, 142, 20, "Single Elimination", false, false);

            AddButton(15, 80, 4005, 4007, 2, GumpButtonType.Reply, 0);
            AddTooltip(Localizations.GetLocalization(35));
            AddHtml(48, 80, 142, 20, "Best of 3", false, false);

            AddButton(15, 110, 4005, 4007, 3, GumpButtonType.Reply, 0);
            AddTooltip(Localizations.GetLocalization(36));
            AddHtml(48, 110, 142, 20, "Last Man Standing", false, false);
        }
    ...
    public class RegisterFightGump : BaseTournamentGump
    {
        ...
        public override void AddGumpLayout()
        {
            ...
            if (Fight.FightType != ArenaTeamType.None && Fight.ArenaFightType != ArenaFightType.LastManStanding)
            {
            ...
            AddHtml(150, 60, 150, 16, Color(LabelColor, "Fight Type"), false, false);
            AddHtml(150, 80, 145, 16, ArenaHelper.GetFightType(Fight.ArenaFightType), false, false);

            AddHtml(0, 180, 300, 16, ColorAndCenter(LabelColor, "Wager"), false, false);
            ...
            AddHtml(0, 255, 300, 16, ColorAndCenter(LabelColor, "Rules"), false, false);
        ...
        public override void OnResponse(RelayInfo button)
        {
            ...
            switch (button.ButtonID)
            {
                ...
                case 7: //Register
                    {
                        if (Fight.ArenaFightType == ArenaFightType.LastManStanding /*&& ArenaTeam.GetTeam(User, ArenaTeamType.Single) != null*/)
                        {
                            ...
                            foreach (Mobile m in eable)
                            {
                                ...
                                if (theirTeam != null && theirTeam != ourTeam && m.NetState != null && !ActionTimer.WaitingAction.ContainsKey(m))
                                {
                                    eligables.Add(m);
                                }
                            }
                        ...
                        else
                        {
                            ...
                            else if (validDuration)
                            {
                                ...
                                if (opp != null && opp is PlayerMobile && !ActionTimer.WaitingAction.ContainsKey(opp))
                                {
        ...
        public class InternalTarget : Target
        {
            ...
            private bool PickOP { get; set; }
            private bool QuickFight { get; set; }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (PickOP)
                {
                    if (targeted is PlayerMobile)
                    {
                        ...
                        else if (!tarTeam.Active)
                            From.SendMessage("That team cannot participate in an Arena fight at this time.");
                        else if (tarTeam == fromTeam)
                            From.SendMessage("You cannot fight your own team!");
                        else if (tarTeam.TeamLeader != null && ActionTimer.WaitingAction.ContainsKey(tarTeam.TeamLeader))
                            From.SendMessage("They are already pending a fight. Try again in a moment.");
                        else if (!ArenaHelper.CheckAvailable(tarTeam))
                            From.SendMessage("They are already fighting or are in queue to fight.");
                        ...
                        else
                        {
                            ...
                            IPooledEnumerable eable = keeper.GetMobilesInRange(30);
                            foreach (Mobile mob in eable)
                            {
                                if (tarTeam.Fighters.Contains(mob) && mob.NetState != null && mob.Alive)
                                    count1++;

                                if (fromTeam.Fighters.Contains(mob) && mob.NetState != null && mob.Alive)
                                    count2++;
                            }
                            eable.Free();
    ...
    public class ConfirmFightGump : BaseTournamentGump
    {
        ...
        public override void AddGumpLayout()
        {
            ...
            if (!Fight.UseOwnGear)
            {
                amend = String.Format("{0} has opted to force standarized gear for this duel. You can select the style of gear to meet your template. Keep in mind, " +
                    "all currenlty worn combat gear will be moved to your bankbox at the start of the duel. Only weapons will remain for use.<br><br>", Challenger.Name);
            }

            if (Fight.Wager > 0)
            {
                amend = String.Format("To match the challenge, {0} will need {0} gold available in their gold account, which will automatically be withdrawn upon accepting the duel.", Fight.FightType == ArenaTeamType.Single ? "you" : "each fighter in your team", Fight.Wager.ToString("N0"));
            }

            string text = String.Format("{0} has challenged your team to a duel in the {1}. You can view the terms and rules, " +
                "and decide if you would like to meet the challenge. {2}", Challenger.Name, System.Name, amend);
